...
const SEED = {
  ...
  regions: [
    ...
    { id:"musandam", nameAr:"مسندم والبريمي والظاهرة",   nameEn:"Musandam, Al Buraimi & Al Dahirah",
      paid:33087.380,   adj:4048.733,   portAmt:37449.515,   portCnt:518,
      collectors:[
  ...
  debtCompanies: [
    { name:"Matrix Debt Collection",        paid:190040.915, adj:31117.278, portAmt:2882018.894, portCnt:23398 },
    { name:"National Center",               paid:99195.084,  adj:4660.167,  portAmt:1014744.033, portCnt:6741  },
    { name:"Compass Risk Support Services", paid:120087.613, adj:10842.512, portAmt:386199.737,  portCnt:3992  },
            ...
            <div style={{marginTop:14,borderTop:"1px solid #f0f4f9",paddingTop:14}}>
              ...
              <button onClick={async()=>{
                ...
                try {
                  ...
                  const dcAmt = SEED.debtCompanies.reduce((s,c)=>s+(c.portAmt||0),0);
                  const hoAmt = SEED.headOffice.reduce((s,c)=>s+(c.portAmt||0),0);
                  const govAmt = SEED.regions.reduce((s,r)=>s+(r.portAmt||0),0);
                  const seedData = {
                    ...SEED,
                    totalPortfolio:{amt:dcAmt+hoAmt+govAmt, cnt:SEED.totalRecords, outstanding:0},
                    _updatedAt:now, lastUpdated:now, complaintsDate:SEED.uploadDate,
                    uploadCount:1, history:[],
                    grandPaid: SEED.regions.reduce((s,r)=>s+(r.paid||0),0)+SEED.debtCompanies.reduce((s,c)=>s+(c.paid||0),0)+SEED.headOffice.reduce((s,h)=>s+(h.paid||0),0),
                    grandAdj: SEED.regions.reduce((s,r)=>s+(r.adj||0),0)+SEED.debtCompanies.reduce((s,c)=>s+(c.adj||0),0)+SEED.headOffice.reduce((s,h)=>s+(h.adj||0),0),
                  };
                  // حفظ في localStorage كـ override
                  try{localStorage.setItem('oneic_dashboard_data', JSON.stringify(seedData));}catch(e){}
                  try{localStorage.setItem('oneic_force_seed','1');}catch(e){}
                  try{localStorage.removeItem('oneic_complaints_branch_map');}catch(e){}
                  try{localStorage.removeItem('oneic_complaints_region_map');}catch(e){}
                  // محاولة Firebase
                  sbUpsert('oneic_data',{payload:seedData}).catch(()=>{});
                  setShowSettings(false);
                  // تحديث مباشر للبيانات بدون reload
                  setData(seedData);
