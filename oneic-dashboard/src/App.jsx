  ...
  useEffect(() => {
    async function load() {
      ...
      if (row?.regions?.length > 0) {
        ...
        // استخدم SEED إذا كانت بيانات Firebase أقدم من SEED
        var seedDate = SEED.uploadDate || "2026-08-02";
        var fbDate   = d.uploadDate || d.complaintsDate || "";
        if (fbDate && fbDate < seedDate) {
          // Firebase أقدم — لا تطغ على البيانات الحالية
          console.log("[ONEIC] Firebase older than SEED, keeping current data");
