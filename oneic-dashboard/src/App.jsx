...
function handleBulkPrint(d, filterFrom, filterTo) {
  ...
  const dateLabel = `${d.dateRange?.from||''} → ${d.dateRange?.to||''}`;
  
  const w = window.open('','_blank','width=900,height=700');
  w.document.write(`<!DOCTYPE html><html lang="ar" dir="rtl">
  <head><meta charset="UTF-8"><title>Bulk Payment Report</title>
  <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;800;900&display=swap" rel="stylesheet">
  <style>*{box-sizing:border-box}body{font-family:Cairo,sans-serif;direction:rtl;padding:20px}
  table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:8px;text-align:right}
  th{background:#1e3a5f;color:#fff}.total{background:#f0f4ff;font-weight:900}
  @media print{@page{size:A4}}</style></head>
  <body>
  <button onclick="window.print()" style="position:fixed;top:10px;left:10px;background:#1e3a5f;color:#fff;border:none;border-radius:8px;padding:8px 16px;cursor:pointer;font-family:Cairo,sans-serif;font-size:13px">🖨️ طباعة</button>
  <h2 style="color:#1e3a5f">💳 Bulk Payment Report</h2>
  <p>${dateLabel} · ${d.totalRecords?.toLocaleString()} دفعة</p>
  <div style="background:#1e3a5f;color:#fff;padding:12px;border-radius:8px;display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:20px">
    <div><div style="font-size:11px;opacity:0.7">إجمالي المدفوع</div><div style="font-size:18px;font-weight:900;color:#86efac">${omrP(totalPaid)}</div></div>
    <div><div style="font-size:11px;opacity:0.7">إجمالي التسويات</div><div style="font-size:18px;font-weight:900;color:#fde68a">${omrP(totalAdj)}</div></div>
    <div><div style="font-size:11px;opacity:0.7">الإجمالي الكلي</div><div style="font-size:22px;font-weight:900">${omrP(totalPaid+totalAdj)}</div></div>
  </div>
  <h3>📅 الأيام</h3>
  <table><thead><tr><th>#</th><th>التاريخ</th><th>المدفوع</th><th>التسويات</th><th>الإجمالي</th><th>دفعات</th></tr></thead>
  <tbody>${filteredDays.map((day,i)=>`<tr><td>${i+1}</td><td>${day.date}</td><td>${omrP(day.paid)}</td><td>${omrP(day.adj)}</td><td style="font-weight:900;color:#1e3a5f">${omrP(day.paid+day.adj)}</td><td>${day.count}</td></tr>`).join('')}
  <tr class="total"><td colspan="2">الإجمالي</td><td>${omrP(totalPaid)}</td><td>${omrP(totalAdj)}</td><td>${omrP(totalPaid+totalAdj)}</td><td>${filteredDays.reduce((s,x)=>s+x.count,0)}</td></tr>
  </tbody></table>
  <h3 style="margin-top:20px">🗺 المناطق</h3>
  <table><thead><tr><th>#</th><th>المنطقة</th><th>المدفوع</th><th>التسويات</th><th>الإجمالي</th></tr></thead>
  <tbody>${(d.byRegion||[]).map((r,i)=>`<tr><td>${i+1}</td><td style="font-weight:700">${r.nameAr||r.nameEn}</td><td>${omrP(r.paid)}</td><td>${omrP(r.adj)}</td><td style="font-weight:900">${omrP(r.paid+r.adj)}</td></tr>`).join('')}
  </tbody></table>
  <p style="text-align:center;color:#aaa;margin-top:20px">ONEIC © 2026</p>
  </body></html>`);
  w.document.close();
...
function Clock({ small }) {
  ...
  return (
    <div style={{textAlign:'right'}}>
      <div style={{fontSize:small?18:26,fontWeight:800,color:'#e85d20',letterSpacing:2,lineHeight:1,fontVariantNumeric:'tabular-nums'}}>
...
function UploadBtn({ onFile, uploading, success, error, small }) {
  ...
  return (
    <div onClick={() => !uploading && ref.current?.click()}
      ...
      style={{border:`2px dashed ${success?'#16a34a':uploading?'#e85d20':'#ddd'}`,borderRadius:12,
        padding:small?'8px 12px':'10px 18px',cursor:uploading?'default':'pointer',
        background:success?'#f0fdf4':uploading?'#fff7f3':'#fff',textAlign:'center',
        minWidth:small?130:160,transition:'all 0.3s'}}>
      ...
      {uploading ? <div style={{color:'#e85d20',fontSize:13,fontWeight:700}}>⏳ جاري التحليل...</div>
       ...
       : <>
           ...
           <div style={{fontSize:small?12:13,color:'#e85d20',fontWeight:700,marginTop:2}}>رفع ملف يومي</div>
         </>}
      {error && <div style={{color:'#dc2626',fontSize:10,marginTop:2,fontWeight:700}}>⚠ {error}</div>}
    </div>
  );
}
...
function EntityCard({ name, paid, adj, color, rank, small }) {
  return (
    <div style={{background:'#fff',borderRadius:13,border:'1.5px solid #f0ece8',
      boxShadow:'0 2px 10px rgba(0,0,0,0.05)',padding:small?'12px 14px':'14px 18px',
    ...
    <div style={{background:'#fff',borderRadius:15,overflow:'hidden',
      ...
      <div style={{background:color,padding:small?'10px 12px':'13px 14px',
        ...
        <div style={{display:'flex',alignItems:'center',gap:6,minWidth:0}}>
          ...
          <span style={{fontSize:small?13:15,fontWeight:900,color:'#fff',lineHeight:1.2}}>{label}</span>
        </div>
        <div style={{background:'rgba(255,255,255,0.25)',borderRadius:20,padding:'2px 8px',
          fontSize:small?12:13,fontWeight:800,color:'#fff'}}>{pct}%</div>
      </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:0,
          ...
          {[['المدفوع','#16a34a',paid],['التسويات','#d97706',adj],['الإجمالي',color,paid+adj]].map(([lbl,clr,val],i)=>(
            <div key={lbl} style={{textAlign:'center',padding:small?'6px 4px':'8px 6px',
              ...
              <div style={{fontSize:small?10:12,color:'#333',fontWeight:800,marginBottom:4}}>{lbl}</div>
              <div style={{fontSize:small?13:15,fontWeight:900,color:clr,lineHeight:1}}>{omr(val)}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div style={{borderRadius:14,overflow:'hidden',
      ...
      <div onClick={onToggle} style={{display:'flex',alignItems:'center',gap:small?10:14,
        ...
        background:open?`${col}08`:'#fff',flexWrap:small?'wrap':'nowrap'}}>
        ...
        <div style={{display:'flex',flex:small?1:0,gap:0,minWidth:small?0:420}}>
        ...
        <div style={{padding:small?'10px':'16px 20px 18px',background:'#fffaf7'}}>
          {(region.collectors||[]).map((c,i)=>(
            <div key={i} style={{display:'grid',
              ...
              {!small && <div style={{textAlign:'center',background:`${col}10`,borderRadius:8,padding:'6px',border:`1px solid ${col}30`}}>
                <div style={{fontSize:10,color:'#888',fontWeight:700,marginBottom:2}}>الإجمالي</div>
                <div style={{fontSize:19,color:col,fontWeight:900}}>{omr(c.paid+c.adj)}</div>
              </div>}
            </div>
          ))}
        </div>
...
function VerifyModal({ pending, onConfirm, onReject }) {
  ...
  const gPaid = (d.regions?.reduce((s,r)=>s+r.paid,0)||0)+(d.debtCompanies?.reduce((s,r)=>s+r.paid,0)||0)+(d.headOffice?.reduce((s,r)=>s+r.paid,0)||0);
  const gAdj  = (d.regions?.reduce((s,r)=>s+r.adj,0)||0)+(d.debtCompanies?.reduce((s,r)=>s+r.adj,0)||0)+(d.headOffice?.reduce((s,r)=>s+r.adj,0)||0);
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',display:'flex',
      ...
      <div style={{background:'#fff',borderRadius:20,width:'100%',maxWidth:480,
        ...
        <div style={{background:'linear-gradient(120deg,#1e3a5f,#2d5a8e)',padding:'18px 24px'}}>
          <div style={{fontSize:18,fontWeight:900,color:'#fff',marginBottom:3}}>✅ تحقق من البيانات</div>
          <div style={{fontSize:12,color:'rgba(255,255,255,0.7)'}}>{pending.fileName} · {pending.fileSize} MB</div>
        </div>
        <div style={{padding:'16px 24px'}}>
          <div style={{display:'flex',justifyContent:'space-between',marginBottom:16}}>
            <div>
              <div style={{fontSize:11,color:'#888',marginBottom:3}}>الإجمالي الكلي</div>
              <div style={{fontSize:26,fontWeight:900,color:'#1e3a5f'}}>{omr(gPaid+gAdj)} <span style={{fontSize:13,color:'#999'}}>OMR</span></div>
            </div>
            <div style={{display:'flex',gap:16}}>
              {[['مدفوع',omr(gPaid),'#16a34a'],['تسويات',omr(gAdj),'#d97706']].map(([l,v,c])=>(
                <div key={l} style={{textAlign:'center'}}>
          ...
          <div style={{fontSize:12,color:'#888',marginBottom:8}}>{d.totalRecords?.toLocaleString()} سجل · {d.regions?.length} محافظات · {d.debtCompanies?.length} شركات</div>
        </div>
        <div style={{display:'flex',gap:12,padding:'0 24px 20px'}}>
          <button onClick={onConfirm} style={{flex:1,background:'#16a34a',color:'#fff',border:'none',
            borderRadius:12,padding:'13px',fontSize:15,fontWeight:900,cursor:'pointer',fontFamily:"'Cairo',sans-serif"}}>
            ✅ تأكيد وتطبيق
          </button>
...
function handlePrint(data) {
  if (!data) return;
  const omrP = n => new Intl.NumberFormat("en-US",{minimumFractionDigits:3,maximumFractionDigits:3}).format(n||0);
  const gPaid = (data.regions?.reduce((s,r)=>s+r.paid,0)||0)+(data.debtCompanies?.reduce((s,r)=>s+r.paid,0)||0)+(data.headOffice?.reduce((s,r)=>s+r.paid,0)||0);
  const gAdj  = (data.regions?.reduce((s,r)=>s+r.adj,0)||0)+(data.debtCompanies?.reduce((s,r)=>s+r.adj,0)||0)+(data.headOffice?.reduce((s,r)=>s+r.adj,0)||0);
  
  const w = window.open('','_blank','width=900,height=700');
  w.document.write(`<!DOCTYPE html><html lang="ar" dir="rtl">
  <head><meta charset="UTF-8"><title>تقرير ONEIC</title>
  <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;800;900&display=swap" rel="stylesheet">
  <style>*{box-sizing:border-box}body{font-family:Cairo,sans-serif;direction:rtl;padding:20px}
  table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:8px;text-align:right}
  th{background:#e85d20;color:#fff}.total{background:#fff7f3;font-weight:900}
  @media print{@page{size:A4}}</style></head>
  <body>
  <button onclick="window.print()" style="position:fixed;top:10px;left:10px;background:#e85d20;color:#fff;border:none;border-radius:8px;padding:8px 16px;cursor:pointer;font-family:Cairo,sans-serif;font-size:13px">🖨️ طباعة</button>
  <h2 style="color:#e85d20">📊 تقرير ONEIC - لوحة تحكم إدارة تحصيل الديون</h2>
  <p>${data.uploadDate||''} · ${data.totalRecords?.toLocaleString()} سجل</p>
  <div style="background:#e85d20;color:#fff;padding:16px;border-radius:10px;margin-bottom:20px;display:flex;justify-content:space-between;align-items:center">
    <div><div style="font-size:12px;opacity:0.8">إجمالي التحصيل الكلي</div><div style="font-size:28px;font-weight:900">${omrP(gPaid+gAdj)} OMR</div></div>
    <div style="display:flex;gap:20px">
      <div><div style="font-size:11px;opacity:0.7">المدفوع</div><div style="font-size:16px;font-weight:900;color:#86efac">${omrP(gPaid)}</div></div>
      <div><div style="font-size:11px;opacity:0.7">التسويات</div><div style="font-size:16px;font-weight:900;color:#fde68a">${omrP(gAdj)}</div></div>
    </div>
  </div>
  <h3>🗺 المحافظات الخمس</h3>
  <table><thead><tr><th>#</th><th>المنطقة</th><th>المدفوع</th><th>التسويات</th><th>الإجمالي</th></tr></thead>
  <tbody>${(data.regions||[]).map((r,i)=>`<tr><td>${i+1}</td><td style="font-weight:700">${r.nameAr||r.label}</td><td>${omrP(r.paid)}</td><td>${omrP(r.adj)}</td><td style="font-weight:900;color:#e85d20">${omrP(r.paid+r.adj)}</td></tr>`).join('')}
  </tbody></table>
  <h3 style="margin-top:20px">🏢 شركات التحصيل</h3>
  <table><thead><tr><th>#</th><th>الشركة</th><th>المدفوع</th><th>التسويات</th><th>الإجمالي</th></tr></thead>
  <tbody>${(data.debtCompanies||[]).map((c,i)=>`<tr><td>${i+1}</td><td style="font-weight:700">${c.name}</td><td>${omrP(c.paid)}</td><td>${omrP(c.adj)}</td><td style="font-weight:900;color:#1a7a6b">${omrP(c.paid+c.adj)}</td></tr>`).join('')}
  </tbody></table>
  <p style="text-align:center;color:#aaa;margin-top:20px">ONEIC © 2026</p>
  </body></html>`);
  w.document.close();
