...
function handlePrint(data) {
  ...
  const amtCells = (paid,adj,color) => `
    <div class="amt-cell"><div class="amt-lbl">المدفوع</div><div class="amt-val" style="color:#16a34a">${omr(paid)}</div></div>
    <div class="sep"></div>
    <div class="amt-cell"><div class="amt-lbl">التسويات</div><div class="amt-val" style="color:#d97706">${omr(adj)}</div></div>
    <div class="sep"></div>
    <div class="amt-cell"><div class="amt-lbl">الإجمالي</div><div class="amt-val" style="color:${color};font-size:12pt">${omr(paid+adj)}</div></div>
  `;

  const regionRows = data.regions.map((r,i) => {
    const col = RCOLS[i%RCOLS.length];
    return `
    <tr>
      <td style="width:8mm;padding:3mm 2mm">
        <div style="width:8mm;height:8mm;border-radius:6px;background:${col};
          display:flex;align-items:center;justify-content:center;
          font-size:11pt;font-weight:900;color:#fff">${i+1}</div>
      </td>
      <td style="padding:3mm 3mm 3mm 0">
        <div style="font-size:11pt;font-weight:900;color:#000">${r.nameAr||r.label||''}</div>
        <div style="font-size:7.5pt;color:#666;margin-top:1mm;font-weight:600">${r.nameEn||''} &nbsp;·&nbsp; ${r.collectors?.length||0} محصّل</div>
      </td>
      <td style="padding:3mm;width:38mm;text-align:center;border-right:1px solid #f0ece8">
        <div style="font-size:7.5pt;color:#555;font-weight:700;margin-bottom:1.5mm">المدفوع</div>
        <div style="font-size:10.5pt;font-weight:900;color:#16a34a">${omr(r.paid)}</div>
      </td>
      <td style="padding:3mm;width:38mm;text-align:center;border-right:1px solid #f0ece8">
        <div style="font-size:7.5pt;color:#555;font-weight:700;margin-bottom:1.5mm">التسويات</div>
        <div style="font-size:10.5pt;font-weight:900;color:#d97706">${omr(r.adj)}</div>
      </td>
      <td style="padding:3mm;width:40mm;text-align:center;background:${col}10;border-radius:4px">
        <div style="font-size:7.5pt;color:#555;font-weight:700;margin-bottom:1.5mm">الإجمالي</div>
        <div style="font-size:12pt;font-weight:900;color:${col}">${omr(r.paid+r.adj)}</div>
      </td>
    </tr>`;
  }).join('');

  const entityRows = (items, color) => items.map((c,i) => `
    <div style="padding:3mm 3mm;border-bottom:1px solid #f5f0ec;page-break-inside:avoid">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:2mm">
        <div style="font-size:10pt;font-weight:900;color:#000">${c.name}</div>
        <div style="width:7mm;height:7mm;border-radius:5px;background:${color};
          display:flex;align-items:center;justify-content:center;
          font-size:9pt;font-weight:900;color:#fff">${i+1}</div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;border:1px solid #f0ece8;border-radius:6px;overflow:hidden">
        <div style="text-align:center;padding:2mm;border-right:1px solid #f0ece8">
          <div style="font-size:7.5pt;color:#555;font-weight:700;margin-bottom:1mm">المدفوع</div>
          <div style="font-size:10pt;font-weight:900;color:#16a34a">${omr(c.paid)}</div>
        </div>
        <div style="text-align:center;padding:2mm;border-right:1px solid #f0ece8">
          <div style="font-size:7.5pt;color:#555;font-weight:700;margin-bottom:1mm">التسويات</div>
          <div style="font-size:10pt;font-weight:900;color:#d97706">${omr(c.adj)}</div>
        </div>
        <div style="text-align:center;padding:2mm;background:${color}0d">
          <div style="font-size:7.5pt;color:#555;font-weight:700;margin-bottom:1mm">الإجمالي</div>
          <div style="font-size:11pt;font-weight:900;color:${color}">${omr(tot(c))}</div>
        </div>
      </div>
    </div>
  `).join('');

  const sumCard = (label, icon, paid, adj, color) => `
    <div style="border-radius:8px;overflow:hidden;border:1.5px solid #f0ece8">
      <div style="background:${color};padding:3mm 4mm;display:flex;justify-content:space-between;align-items:center">
        ...
        <span style="background:rgba(255,255,255,0.25);border-radius:20px;padding:1mm 3mm;font-size:9pt;font-weight:800;color:#fff">${pct(paid+adj)}</span>
      </div>
      <div style="padding:3mm 4mm">
        ${[["المدفوع","#16a34a",omr(paid)],["التسويات","#d97706",omr(adj)],["الإجمالي",color,omr(paid+adj)]].map(([l,c,v])=>`
        <div style="display:flex;justify-content:space-between;align-items:center;padding:1.5mm 0;border-bottom:1px solid #f5f0ec">
          <span style="font-size:8.5pt;color:#333;font-weight:700">${l}</span>
          <span style="font-size:10pt;font-weight:900;color:${c}">${v}</span>
        </div>`).join('')}
      </div>
    </div>`;
...
<style>
  *{box-sizing:border-box;margin:0;padding:0;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
  ...
  .page{width:210mm;min-height:297mm;margin:8mm auto;background:#fff;padding:11mm 10mm;box-shadow:0 4px 32px rgba(0,0,0,0.15)}
  @media print{
  ...
  .sep{width:1px;background:#f0ece8}
</style>
</head>
<body>
<button class="no-print" onclick="window.print()" style="position:fixed;top:10mm;left:10mm;background:#e85d20;color:#fff;border:none;border-radius:10px;padding:10px 24px;font-size:14px;font-weight:800;cursor:pointer;font-family:'Cairo',sans-serif;box-shadow:0 4px 16px rgba(232,93,32,0.4);z-index:999">🖨️ طباعة / حفظ PDF</button>

<div class="page">

  <!-- HEADER -->
  <div style="display:flex;justify-content:space-between;align-items:center;padding-bottom:4mm;border-bottom:3px solid #e85d20;margin-bottom:5mm">
    <div style="display:flex;align-items:center;gap:10px">
      <img src="${LOGO_B64}" alt="ONEIC" style="height:48px;object-fit:contain">
      <div>
        <div style="font-size:17pt;font-weight:900;color:#e85d20;line-height:1.1">لوحة تحكم إدارة تحصيل الديون</div>
        <div style="font-size:8pt;color:#444;font-weight:700;margin-top:2px">Debt Collection Management Dashboard &nbsp;·&nbsp; ${(data.totalRecords||0).toLocaleString()} سجل &nbsp;·&nbsp; آخر تحديث: ${data.uploadDate||''}</div>
      </div>
    </div>
    <div style="text-align:left">
      <div style="font-size:8pt;color:#555">تاريخ الطباعة: ${new Date().toLocaleDateString('ar-OM')}</div>
      <div style="font-size:10pt;font-weight:800;color:#e85d20;margin-top:1mm">ONEIC © 2026</div>
    </div>
  </div>

  <!-- BANNER -->
  <div style="background:linear-gradient(120deg,#e85d20,#c44b10,#a83808);border-radius:10px;padding:5mm 6mm;margin-bottom:5mm;display:flex;justify-content:space-between;align-items:center">
    <div>
      <div style="font-size:9pt;color:rgba(255,255,255,0.8);font-weight:700;margin-bottom:2mm">إجمالي التحصيل الكلي</div>
      <div style="display:flex;align-items:baseline;gap:6px">
        <span style="font-size:28pt;font-weight:900;color:#fff;letter-spacing:-1px;line-height:1">${omr(grand)}</span>
        <span style="font-size:13pt;color:rgba(255,255,255,0.75);font-weight:700">OMR</span>
      </div>
    </div>
    <div style="display:flex;gap:6mm">
      ${[["إجمالي المدفوع",omr(gPaid),"#fff"],["إجمالي التسويات",omr(gAdj),"#fde68a"],["عدد السجلات",(data.totalRecords||0).toLocaleString(),"#bfdbfe"]].map(([l,v,c])=>`
      <div style="text-align:center">
        <div style="font-size:8pt;color:rgba(255,255,255,0.75);font-weight:700;margin-bottom:2mm">${l}</div>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:3mm;margin-bottom:5mm">
    ${sumCard("المحافظات الخمس","🗺",govPaid,govAdj,"#e85d20")}
    ${sumCard("شركات التحصيل","🏢",dcPaid,dcAdj,"#1a7a6b")}
    ${sumCard("المكتب الرئيسي","🏛",hoPaid,hoAdj,"#6c3fa0")}
  </div>

  <!-- REGIONS -->
  <div style="margin-bottom:5mm">
    <div style="background:linear-gradient(120deg,#e85d20,#c44b10);border-radius:8px 8px 0 0;padding:3mm 4mm;display:flex;justify-content:space-between;align-items:center">
      <span style="font-size:12pt;font-weight:900;color:#fff">🗺 المحافظات الخمس</span>
      <div style="display:flex;gap:6mm">
        ${[["المدفوع",omr(govPaid),"#fff"],["التسويات",omr(govAdj),"#fde68a"],["الإجمالي",omr(govPaid+govAdj),"#fff"]].map(([l,v,c])=>`<div style="text-align:center"><div style="font-size:7.5pt;color:rgba(255,255,255,0.75);font-weight:700;margin-bottom:1mm">${l}</div><div style="font-size:10pt;font-weight:900;color:${c}">${v}</div></div>`).join('')}
      </div>
    </div>
    <table style="border:1.5px solid #f0ece8;border-top:none;border-radius:0 0 8px 8px;overflow:hidden">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:4mm">
    <div style="border-radius:8px;overflow:hidden;border:1.5px solid #f0ece8">
      <div style="background:linear-gradient(120deg,#1a7a6b,#0f5a4d);padding:3mm 4mm;display:flex;justify-content:space-between;align-items:center">
        <span style="font-size:12pt;font-weight:900;color:#fff">🏢 شركات التحصيل</span>
        <div style="text-align:left"><div style="font-size:7.5pt;color:rgba(255,255,255,0.75);font-weight:700">الإجمالي</div><div style="font-size:11pt;font-weight:900;color:#fff">${omr(dcPaid+dcAdj)}</div></div>
      </div>
      ${entityRows(data.debtCompanies,"#1a7a6b")}
    </div>
    <div style="border-radius:8px;overflow:hidden;border:1.5px solid #f0ece8">
      <div style="background:linear-gradient(120deg,#6c3fa0,#4e2a80);padding:3mm 4mm;display:flex;justify-content:space-between;align-items:center">
        <span style="font-size:12pt;font-weight:900;color:#fff">🏛 المكتب الرئيسي</span>
        <div style="text-align:left"><div style="font-size:7.5pt;color:rgba(255,255,255,0.75);font-weight:700">الإجمالي</div><div style="font-size:11pt;font-weight:900;color:#fff">${omr(hoPaid+hoAdj)}</div></div>
      </div>
      ${entityRows(data.headOffice,"#6c3fa0")}
    </div>
  </div>

  <!-- FOOTER -->
  <div style="text-align:center;font-size:8pt;color:#aaa;margin-top:5mm;padding-top:3mm;border-top:1px solid #eee">
    ONEIC — لوحة تحكم إدارة تحصيل الديون &nbsp;·&nbsp; تقرير بتاريخ ${data.uploadDate||''} &nbsp;·&nbsp; © 2026
  </div>
