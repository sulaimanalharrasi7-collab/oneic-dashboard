...
function parseXLS(file) {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        const text = new TextDecoder("utf-16-le").decode(new Uint8Array(e.target.result).slice(5));
...
function Clock() {
  ...
  return (
    <div style={{textAlign:"right"}}>
      <div style={{fontSize:20,fontWeight:900,color:"#f97316",letterSpacing:2,fontVariantNumeric:"tabular-nums"}}>
        {t.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:false})}
      </div>
      <div style={{fontSize:9,color:"rgba(255,255,255,0.35)",letterSpacing:0.5}}>
        {t.toLocaleDateString("ar-OM",{weekday:"short",year:"numeric",month:"short",day:"numeric"})}
      </div>
    </div>
  );
}

// ── Region card with dropdown ─────────────────────────────────────────────────
const REG_COLORS = ["#f97316","#fb923c","#fdba74","#fbbf24","#f59e0b"];

function RegionCard({ region, idx, open, onToggle }) {
  const color = REG_COLORS[idx];
  const total = region.paid + region.adj;
  const maxVal = region.collectors.length ? Math.max(...region.collectors.map(c=>c.paid+c.adj), 1) : 1;
    ...
    <div style={{
      ...
      >
        ...
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div>
            <div style={{fontSize:15,fontWeight:800,color:"#fff",lineHeight:1.2}}>{region.nameAr}</div>
            <div style={{fontSize:10,color:"rgba(255,255,255,0.35)",marginTop:2}}>{region.nameEn}</div>
          </div>
        </div>
        ...
        <div style={{display:"flex",gap:32,alignItems:"center"}}>
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:9,color:"rgba(255,255,255,0.4)",fontWeight:700,letterSpacing:0.8,marginBottom:3}}>المدفوع</div>
            <div style={{fontSize:16,fontWeight:800,color:"#86efac"}}>{omr(region.paid)}</div>
          </div>
          <div style={{width:1,height:32,background:"rgba(255,255,255,0.08)"}}/>
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:9,color:"rgba(255,255,255,0.4)",fontWeight:700,letterSpacing:0.8,marginBottom:3}}>التسويات</div>
            <div style={{fontSize:16,fontWeight:800,color:"#fde68a"}}>{omr(region.adj)}</div>
          </div>
          <div style={{width:1,height:32,background:"rgba(255,255,255,0.08)"}}/>
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:9,color:"rgba(255,255,255,0.4)",fontWeight:700,letterSpacing:0.8,marginBottom:3}}>الإجمالي</div>
            <div style={{fontSize:18,fontWeight:900,color}}>{omr(total)}</div>
          </div>
        </div>
      ...
      {/* ── Collector dropdown ── */}
      {open && (
        <div style={{borderTop:`1px solid rgba(255,255,255,0.07)`,padding:"16px 22px 18px"}}>
          {/* Header row */}
          <div style={{
            display:"grid",gridTemplateColumns:"30px 1fr 110px 110px 110px 80px",gap:8,
            fontSize:9,color:"rgba(255,255,255,0.35)",fontWeight:700,letterSpacing:0.8,
            padding:"0 0 8px",borderBottom:"1px solid rgba(255,255,255,0.06)",marginBottom:8
          }}>
            <span>#</span><span>المحصّل</span>
          ...
          {region.collectors.map((c,i) => {
            ...
            const pct = maxVal > 0 ? ((ct/maxVal)*100).toFixed(0) : 0;
            return (
              <div key={i} style={{
                display:"grid",gridTemplateColumns:"30px 1fr 110px 110px 110px 80px",
                gap:8, alignItems:"center", padding:"7px 0",
                borderBottom:"1px solid rgba(255,255,255,0.04)"
              }}>
                <span style={{fontSize:10,color:"rgba(255,255,255,0.3)",fontWeight:600}}>{i+1}</span>
                <span style={{fontSize:12,color:"#e2e8f0",fontWeight:600,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{c.name}</span>
                <span style={{fontSize:12,color:"#86efac",fontWeight:700,textAlign:"right"}}>{omr(c.paid)}</span>
                <span style={{fontSize:12,color:"#fde68a",fontWeight:700,textAlign:"right"}}>{omr(c.adj)}</span>
                <span style={{fontSize:13,color:"#fff",fontWeight:800,textAlign:"right"}}>{omr(ct)}</span>
                <div style={{display:"flex",alignItems:"center",gap:4,justifyContent:"flex-end"}}>
                  <div style={{width:28,height:4,background:"rgba(255,255,255,0.08)",borderRadius:2,overflow:"hidden"}}>
                    <div style={{width:pct+"%",height:"100%",background:color,borderRadius:2}}/>
                  </div>
                  <span style={{fontSize:9,color:"rgba(255,255,255,0.35)",width:24,textAlign:"right"}}>{pct}%</span>
                </div>
              </div>
            );
          })}
          ...
          <div style={{
            display:"grid",gridTemplateColumns:"30px 1fr 110px 110px 110px 80px",
            gap:8,padding:"10px 0 0",borderTop:`1px solid ${color}44`,marginTop:6
          }}>
            <span/><span style={{fontSize:11,color:color,fontWeight:800}}>الإجمالي</span>
            <span style={{fontSize:12,color:"#86efac",fontWeight:800,textAlign:"right"}}>{omr(region.paid)}</span>
            <span style={{fontSize:12,color:"#fde68a",fontWeight:800,textAlign:"right"}}>{omr(region.adj)}</span>
            <span style={{fontSize:14,color:color,fontWeight:900,textAlign:"right"}}>{omr(total)}</span>
            <span/>
          </div>
        </div>
      )}
...
// ── Simple entity card (no collectors) ───────────────────────────────────────
  ...
  return (
    <div style={{
      ...
      <div style={{display:"flex",gap:24,alignItems:"center"}}>
        <div style={{textAlign:"center"}}>
          <div style={{fontSize:9,color:"rgba(255,255,255,0.4)",fontWeight:700,letterSpacing:0.8,marginBottom:2}}>المدفوع</div>
          <div style={{fontSize:15,fontWeight:800,color:"#86efac"}}>{omr(paid)}</div>
        </div>
        <div style={{width:1,height:28,background:"rgba(255,255,255,0.08)"}}/>
        <div style={{textAlign:"center"}}>
          <div style={{fontSize:9,color:"rgba(255,255,255,0.4)",fontWeight:700,letterSpacing:0.8,marginBottom:2}}>التسويات</div>
          <div style={{fontSize:15,fontWeight:800,color:"#fde68a"}}>{omr(adj)}</div>
        </div>
        <div style={{width:1,height:28,background:"rgba(255,255,255,0.08)"}}/>
        <div style={{textAlign:"center"}}>
          <div style={{fontSize:9,color:"rgba(255,255,255,0.4)",fontWeight:700,letterSpacing:0.8,marginBottom:2}}>الإجمالي</div>
          <div style={{fontSize:16,fontWeight:900,color}}>{omr(total)}</div>
        </div>
      </div>
    </div>
  );
...
function SectionTitle({ title, sub, color }) {
  return (
    <div style={{
      ...
      padding:"10px 0 8px",borderBottom:`2px solid ${color}33`,marginBottom:10
    }}>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <div style={{width:4,height:20,background:color,borderRadius:2}}/>
        <span style={{fontSize:15,fontWeight:900,color:"#fff",letterSpacing:0.3}}>{title}</span>
      </div>
      {sub && <span style={{fontSize:11,color:"rgba(255,255,255,0.4)",fontWeight:600}}>{sub}</span>}
    </div>
  );
}

// ── Grand total banner ────────────────────────────────────────────────────────
function GrandBanner({ paid, adj }) {
  const total = paid + adj;
  return (
    <div style={{
      ...
      display:"flex",justifyContent:"space-between",alignItems:"center",
      boxShadow:"0 4px 32px rgba(249,115,22,0.3)",
      border:"1px solid rgba(249,115,22,0.4)",
      marginBottom:16
    }}>
      <div>
        <div style={{fontSize:10,color:"rgba(255,255,255,0.65)",fontWeight:700,letterSpacing:1.5,marginBottom:4}}>إجمالي التحصيل الكلي</div>
        <div style={{display:"flex",alignItems:"baseline",gap:8}}>
          <span style={{fontSize:38,fontWeight:900,color:"#fff",letterSpacing:-1}}>{omr(total)}</span>
          <span style={{fontSize:14,color:"rgba(255,255,255,0.55)",fontWeight:500}}>OMR</span>
        </div>
      </div>
      <div style={{display:"flex",gap:36}}>
        {[["💰 إجمالي مدفوع",paid,"#fff"],["📊 إجمالي تسويات",adj,"#fde68a"]].map(([l,v,c])=>(
          <div key={l} style={{textAlign:"center"}}>
            <div style={{fontSize:10,color:"rgba(255,255,255,0.55)",marginBottom:4}}>{l}</div>
            <div style={{fontSize:22,fontWeight:900,color:c}}>{omr(v)}</div>
          </div>
        ))}
      </div>
    </div>
...
function UploadBtn({ onFile, uploading, success, error }) {
  ...
  return (
    <div
      ...
      style={{
        border:`1.5px dashed ${success?"#22c55e":uploading?"#f97316":"rgba(249,115,22,0.45)"}`,
        borderRadius:10,padding:"8px 16px",cursor:uploading?"default":"pointer",
        background:success?"rgba(34,197,94,0.07)":uploading?"rgba(249,115,22,0.07)":"rgba(249,115,22,0.04)",
        textAlign:"center",minWidth:175,transition:"all 0.25s"
      }}
    >
      ...
      {uploading
        ? <div style={{color:"#f97316",fontSize:12,fontWeight:700}}>⏳ جاري التحليل...</div>
        : success
        ? <div style={{color:"#22c55e",fontSize:12,fontWeight:700}}>✅ تم التحديث</div>
        : <>
            <div style={{fontSize:16}}>📂</div>
            <div style={{fontSize:11,color:"#f97316",fontWeight:700}}>رفع ملف يومي</div>
            <div style={{fontSize:9,color:"rgba(255,255,255,0.3)",marginTop:1}}>يستبدل البيانات تلقائياً</div>
          </>
      }
      {error && <div style={{color:"#f87171",fontSize:9,marginTop:3}}>⚠ {error}</div>}
    </div>
  );
}
...
export default function Dashboard() {
  ...
  return (
    <div style={{
      ...
      background:"linear-gradient(135deg,#0d0d1f 0%,#1a0d0d 55%,#0d1a1a 100%)",
      ...
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        ::-webkit-scrollbar-thumb{background:rgba(249,115,22,0.5);border-radius:2px}
        ::-webkit-scrollbar-track{background:transparent}
        @keyframes glow{0%,100%{box-shadow:0 0 20px rgba(249,115,22,0.25)}50%{box-shadow:0 0 36px rgba(249,115,22,0.5)}}
      `}</style>
      ...
      <div style={{
        ...
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <img src={LOGO} alt="ONEIC" style={{height:42,objectFit:"contain",filter:"drop-shadow(0 0 8px rgba(249,115,22,0.45))"}}/>
          <div style={{borderRight:"1px solid rgba(255,255,255,0.1)",paddingRight:14}}>
            <div style={{fontSize:19,fontWeight:900,color:"#fff",letterSpacing:0.3}}>لوحة تحكم التحصيل</div>
      ...
      <div style={{flex:1,overflow:"auto",padding:"16px 22px 20px"}}>
        ...
        {/* Grand Total */}
        <GrandBanner paid={grandPaid} adj={grandAdj}/>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:24,alignItems:"start"}}>
          ...
          <div>
            <SectionTitle
              title="المحافظات الخمس"
              sub={`إجمالي: ${omr(govPaid+govAdj)}`}
              color="#f97316"
            />
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {data.regions.map((r,i) => (
                <RegionCard
                  key={r.id} region={r} idx={i}
          ...
          <div>
            <SectionTitle
              title="شركات التحصيل"
              sub={`إجمالي: ${omr(dcPaid+dcAdj)}`}
              color="#22d3ee"
            />
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {data.debtCompanies.map((c,i) => (
                <EntityCard key={c.name} name={c.name} paid={c.paid} adj={c.adj} color="#22d3ee" rank={i+1}/>
              ))}
            </div>
            ...
            <div style={{marginTop:28}}>
              <SectionTitle
                ...
                sub={`إجمالي: ${omr(hoPaid+hoAdj)}`}
                color="#a78bfa"
              />
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {data.headOffice.map((c,i) => (
                  <EntityCard key={c.name} name={c.name} paid={c.paid} adj={c.adj} color="#a78bfa" rank={i+1}/>
                ))}
              </div>
            </div>
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <SectionTitle title="ملخص الأقسام" color="#86efac"/>
            {[
              {label:"المحافظات", paid:govPaid, adj:govAdj, color:"#f97316", icon:"🗺"},
              {label:"شركات التحصيل", paid:dcPaid, adj:dcAdj, color:"#22d3ee", icon:"🏢"},
              {label:"المكتب الرئيسي", paid:hoPaid, adj:hoAdj, color:"#a78bfa", icon:"🏛"}
            ].map(item => (
              <div key={item.label} style={{
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                  <span style={{fontSize:13,fontWeight:800,color:"#fff"}}>{item.icon} {item.label}</span>
                  <span style={{fontSize:10,color:"rgba(255,255,255,0.35)"}}>OMR</span>
                </div>
                <div style={{display:"flex",gap:20}}>
                  <div>
                    <div style={{fontSize:9,color:"rgba(255,255,255,0.4)",fontWeight:700,letterSpacing:0.8,marginBottom:4}}>المدفوع</div>
                    <div style={{fontSize:17,fontWeight:800,color:"#86efac"}}>{omr(item.paid)}</div>
                  </div>
                  <div>
                    <div style={{fontSize:9,color:"rgba(255,255,255,0.4)",fontWeight:700,letterSpacing:0.8,marginBottom:4}}>التسويات</div>
                    <div style={{fontSize:17,fontWeight:800,color:"#fde68a"}}>{omr(item.adj)}</div>
                  </div>
                  <div>
                    <div style={{fontSize:9,color:"rgba(255,255,255,0.4)",fontWeight:700,letterSpacing:0.8,marginBottom:4}}>الإجمالي</div>
                    <div style={{fontSize:19,fontWeight:900,color:item.color}}>{omr(item.paid+item.adj)}</div>
                  </div>
                </div>
                {/* mini bar */}
                <div style={{marginTop:12,background:"rgba(255,255,255,0.07)",borderRadius:4,height:5,overflow:"hidden"}}>
                  <div style={{
                    width:((item.paid+item.adj)/(grandPaid+grandAdj)*100)+"%",
                    height:"100%",background:item.color,borderRadius:4,transition:"width 0.8s"
                  }}/>
                </div>
                <div style={{fontSize:9,color:"rgba(255,255,255,0.3)",marginTop:4,textAlign:"left"}}>
                  {((item.paid+item.adj)/(grandPaid+grandAdj)*100).toFixed(1)}% من الإجمالي
                </div>
              </div>
            ))}
          </div>
